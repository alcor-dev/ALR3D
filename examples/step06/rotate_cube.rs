...
impl State {
    ...
    pub fn resize(&mut self, new_size: winit::dpi::PhysicalSize<u32>) {
        if new_size.width > 0 && new_size.height > 0 {
            ...
            self.project_mat = transforms::
                ...
                let mvp_mat = self.project_mat * self.model_mat;
                let mvp_ref:&[f32; 16] = mvp_mat.as_ref();
    ...
    unsafe fn update(&mut self, dt: std::time::Duration){
        let dt = (ANIMATION_SPEED * MULTIPLIER) * dt.as_secs_f32();
        let model_mat = transforms::create_transforms([0.0,0.0,0.0],
            [ dt.sin(), dt.cos() , 0.0 ], [1.0, 1.0, 1.0]);
        //con dt simplemente rota, pero con dt.sin o dt.cos, acelera y luego cambia, pero no mantiene velocidad constante
...
fn sum_multiplier(num:f32) {
    unsafe{
        MULTIPLIER += num
    }
}

fn sub_multiplier(num:f32) {
    unsafe{
        MULTIPLIER -= num
    }
}
            ...
            Event::WindowEvent {
                ...
                if !state.input(event) {
                    match event {
                        ...
                        //cuando pulsas D, aumenta la velocidad de rotación de forma positiva
                        WindowEvent::KeyboardInput {
                            input:
                                KeyboardInput {
                                    state: ElementState::Pressed,
                        ...
                        //cuando pulsas A, aumenta la velocidad de rotación de forma negativa
                        WindowEvent::KeyboardInput {
                            input:
                                KeyboardInput {
                                    state: ElementState::Pressed,
                        ...
                        //reinicia la velocidad de rotación a su velocidad inicial
                        WindowEvent::KeyboardInput {
                            input:
                                KeyboardInput {
                                    state: ElementState::Pressed,
